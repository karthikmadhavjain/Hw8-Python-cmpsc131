...
def get_misspelled_linear(dictionary, text):
  ...
  allmisspelled = []
  for word in text:
    evaluatedword = word.lower()
    boolean= False
    for wordind in dictionary:
      if wordind == evaluatedword:
        boolean= True
        break
    if boolean == False:
      allmisspelled.append(word)
  return allmisspelled 
...
def get_misspelled_binary(dictionary, text):
  ...
  for word in text:
    ...
    while start <= end:
      mid=((start+end)//2)
      if dictionary[mid] == evaluatedword:
        boolean= True
        break
      elif dictionary[mid] > evaluatedword:
        end= mid-1
      else:
        start= mid+1              
    if boolean == False:
